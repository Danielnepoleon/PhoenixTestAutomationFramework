...
public class LoginApiTest {
	...
	public void loginApiTest() throws IOException {
		given().spec(requestSpec()).body(credentials).when().post("login").then().log().all().spec(responseSpec_OK())
				.and().body("message", equalTo("Success")).and().body("data.token", notNullValue()).and()
