...
public class LoginApiDataDrivenTest {
	...
	@Test(description = "Verify if login api is working for user FD", groups = { "api", "regression",
			"smoke" }, dataProviderClass = DataProviderUtils.class, dataProvider = "LoginApiDataProvider")
	...
	public void loginApiJsonDataTest(Usercredentials userCredentials) {
		authService.login(userCredentials).then().log().all().spec(responseSpec_OK()).and()
